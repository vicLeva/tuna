#!/usr/bin/env bash
# bench_ecoli_kmc_only.sh — KMC count + dump on E. coli, matching bench_ecoli_n_sweep.sh
# Outputs same CSV format; merge with existing tuna results locally.
#
# Usage: bash bench_ecoli_kmc_only.sh [THREADS] [K] [M] [KMC_MEM_GB]
...
N_VALUES=(1 2 5 10 20 50 100 200 500 1000 1500 2000 2500 3000 3600)

RESULTS="$WORK/kmc_results_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS"

TOTAL_FILES=$(wc -l < "$FOF")
echo "=== E. coli KMC-only (count+dump): k=$K  m=$M  t=$THREADS  mem=${KMC_MEM_GB}GB ==="
