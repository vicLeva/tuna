...
// Single-owner counterpart used by non-concurrent streaming tables.
template <typename T_key_, typename T_val_, typename T_hasher_>
...
public:
    Serial_Hash_Table(std::size_t max_n, double load_factor = lf_default,
                      T_hasher_ hasher = T_hasher_());
    ...
    Iterator iterator() { return Iterator(*this, 1, 0); }
    Iterator iterator(std::size_t it_count, std::size_t it_id)
    {
        return Iterator(*this, it_count, it_id);
    }
};
...
template <typename T_key_, typename T_val_, typename T_hasher_>
inline Serial_Hash_Table<T_key_, T_val_, T_hasher_>::Serial_Hash_Table(
    const std::size_t max_n, const double load_factor, const T_hasher_ hasher):
      empty_key_()
    , hash(hasher)
    , capacity_(ceil_pow_2(static_cast<std::size_t>(std::ceil(max_n / load_factor))))
    , idx_wrapper_mask(capacity_ - 1)
...
template <typename T_key_, typename T_val_, typename T_hasher_>
inline const T_val_* Serial_Hash_Table<T_key_, T_val_, T_hasher_>::insert(
    const T_key_ key, const T_val_ val)
{
    for(std::size_t tried = 0, i = hash_to_idx(hash(key));
        tried < capacity_; ++tried, i = next_index(i))
    {
        if(T[i].key == empty_key_)
        {
            T[i].key = key;
...
template <typename T_key_, typename T_val_, typename T_hasher_>
inline std::optional<T_val_> Serial_Hash_Table<T_key_, T_val_, T_hasher_>::upsert(
    const T_key_ key, const T_val_ val)
{
    for(std::size_t tried = 0, i = hash_to_idx(hash(key));
        tried < capacity_; ++tried, i = next_index(i))
...
template <typename T_key_, typename T_val_, typename T_hasher_>
inline const T_val_* Serial_Hash_Table<T_key_, T_val_, T_hasher_>::find(
    const T_key_ key) const
{
    for(std::size_t tried = 0, i = hash_to_idx(hash(key));
        tried < capacity_; ++tried, i = next_index(i))
    {
...
template <typename T_key_, typename T_val_, typename T_hasher_>
template <typename F>
inline std::optional<T_val_> Serial_Hash_Table<T_key_, T_val_, T_hasher_>::find_and_update(
    const T_key_ key, const F& f)
{
    for(std::size_t tried = 0, i = hash_to_idx(hash(key));
        tried < capacity_; ++tried, i = next_index(i))
    {
...
template <typename T_key_, typename T_val_, typename T_hasher_>
inline Serial_Hash_Table<T_key_, T_val_, T_hasher_>::Iterator::Iterator(
    Serial_Hash_Table& M, std::size_t it_count, std::size_t it_id):
      M(M)
{
    const auto range_sz = (M.capacity_ + it_count - 1) / it_count;
    idx = it_id * range_sz;
...
template <typename T_key_, typename T_val_, typename T_hasher_>
inline bool Serial_Hash_Table<T_key_, T_val_, T_hasher_>::Iterator::next(
    T_key_& key, T_val_& val)
{
    for(; idx < end; ++idx)
        if(M.T[idx].key != M.empty_key_)
        {
