...
int run(const Config& cfg)
{
    ...
    if (use_mem_pipeline) {
        ...
        auto do_count_mem = [&](auto canonical_tag) {
            ...
            if (cfg.count_only)
                return count_and_write_mem<k, m, C>(cfg, stats.kmers, part_bufs, nullptr, nullptr);
            if (cfg.output_kff) {
                KffOutput kff_out(cfg.output_file, cfg.k);
            ...
            return count_and_write_mem<k, m, C>(cfg, stats.kmers, part_bufs, &tsv_out, nullptr);
        };
    ...
    auto do_count = [&](auto canonical_tag) {
        constexpr bool C = decltype(canonical_tag)::value;
        if (cfg.count_only)
            return count_and_write<k, m, C>(cfg, stats.kmers, nullptr, nullptr);
        if (cfg.output_kff) {
            KffOutput kff_out(cfg.output_file, cfg.k);
        ...
        return count_and_write<k, m, C>(cfg, stats.kmers, &tsv_out, nullptr);
    };
    const auto [total_inserted, total_written] = cfg.canonical
